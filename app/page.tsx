...
export default function Home() {
  const inputRef = useRef<HTMLInputElement>(null); const bookRef = useRef<any>(null); const bookElementRef = useRef<HTMLDivElement>(null); const viewerRef = useRef<HTMLDivElement>(null); const audioRef = useRef<AudioContext | null>(null);
  ...
  const playFlip = useCallback(() => {
    if (!sound) return; const AudioCtor = window.AudioContext || (window as any).webkitAudioContext; const context = audioRef.current ?? new AudioCtor(); audioRef.current = context; const now = context.currentTime; const length = Math.floor(context.sampleRate * .16); const buffer = context.createBuffer(1, length, context.sampleRate); const data = buffer.getChannelData(0);
    for (let i = 0; i < length; i += 1) data[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / length, 2);
    const source = context.createBufferSource(); const filter = context.createBiquadFilter(); const gain = context.createGain(); filter.type = 'bandpass'; filter.frequency.setValueAtTime(1100, now); filter.frequency.exponentialRampToValueAtTime(280, now + .16); gain.gain.setValueAtTime(.055, now); gain.gain.exponentialRampToValueAtTime(.001, now + .16); source.buffer = buffer; source.connect(filter).connect(gain).connect(context.destination); source.start(now);
  }, [sound]);
  useEffect(() => {
  ...
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if (event.key === 'ArrowLeft') flip('prev'); if (event.key === 'ArrowRight') flip('next'); }; window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey); }, [flip]);
