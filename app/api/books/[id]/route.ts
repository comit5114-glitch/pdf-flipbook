import { db, deletePdf, jsonError, signedPdfUrls, type TemporaryBook } from '@/lib/supabase-server';
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const pdfUrls = await signedPdfUrls(book.storage_path);
    return Response.json({ book: { id: book.id, title: book.title, expiresAt: book.expires_at, lastPage: book.last_page || 1 }, pdfUrl: pdfUrls[0], pdfUrls });
