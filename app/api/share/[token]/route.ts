import { db, jsonError, signedPdfUrls, type TemporaryBook } from '@/lib/supabase-server';
...
export async function GET(_request: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    return Response.json({ book: { title: book.title, expiresAt: book.expires_at }, pdfUrl: pdfUrls[0], pdfUrls });
